...
namespace POINT_OF_SALE
{
    public partial class Frm_login : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT * FROM tbl_user WHERE username = '"+txt_username.Text+"' AND password = '"+txt_password.Text+"' ", conn);
            ...
            } else
            {
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            MessageBox.Show("Kamu berhasil Login sebagai " + dt.Rows[i][3], "Login", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            adminDashboard.Show();
                            Hide();
                        }
                        else if (dt.Rows[i]["level"].ToString() == "Cashier")
                        {
                            MessageBox.Show("Kamu berhasil Login sebagai " + dt.Rows[i][3], "Login", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            cashier.Show();
                            Hide();
